...
def test_daily_entry_sums_online_and_dine_in_with_order_counts():
    # Tribeca has a pos_name ("DEL KK Nexus Select CityWalk Mall POS") distinct from its
    # up_name ("DEL KK Nexus Select CityWalk Mall Online") — dine_in_rows arrive keyed by pos_name and
    # must be mapped back to up_name.
    online_rows = [
        {"order_date": "2026-08-17", "store_name": "DEL KK Nexus Select CityWalk Mall Online", "channel": "swiggy", "revenue": "1000", "order_count": "10"},
        {"order_date": "2026-08-17", "store_name": "DEL KK Nexus Select CityWalk Mall Online", "channel": "zomato", "revenue": "500", "order_count": "5"},
    ]
    dine_in_rows = [
        {"order_date": "2026-08-17", "store_name": "DEL KK Nexus Select CityWalk Mall POS", "revenue": "200", "order_count": "4"},
    ]
    launch_date_rows = [{"store_name": "DEL KK Nexus Select CityWalk Mall Online", "launch_date": "2025-03-12"}]

    payload = build_dashboard_payload(online_rows, dine_in_rows, [], launch_date_rows, date(2026, 8, 18))
    tribeca = next(s for s in payload["stores"] if s["up_name"] == "DEL KK Nexus Select CityWalk Mall Online")
...
def test_today_excluded_from_daily_full_day_rule():
    online_rows = [
        {"order_date": "2026-08-18", "store_name": "DEL KK GK1 Online", "channel": "swiggy", "revenue": "999", "order_count": "3"},
    ]
    payload = build_dashboard_payload(online_rows, [], [], [], date(2026, 8, 18))
    pimpri = next(s for s in payload["stores"] if s["up_name"] == "DEL KK GK1 Online")
...
def test_mtd_only_sums_current_month():
    online_rows = [
        {"order_date": "2026-07-31", "store_name": "DEL KK GK1 Online", "channel": "swiggy", "revenue": "5000", "order_count": "20"},
        {"order_date": "2026-08-01", "store_name": "DEL KK GK1 Online", "channel": "swiggy", "revenue": "300", "order_count": "2"},
    ]
    payload = build_dashboard_payload(online_rows, [], [], [], date(2026, 8, 18))
    pimpri = next(s for s in payload["stores"] if s["up_name"] == "DEL KK GK1 Online")
...
def test_all_thirteen_stores_present():
    payload = build_dashboard_payload([], [], [], [], date(2026, 8, 18))
    assert len(payload["stores"]) == 39


def test_ops_computed_cancellation_and_kpt():
    ops_metric_rows = [
        {"order_date": "2026-08-17", "store_name": "DEL KK GK1 Online", "channel": "swiggy",
         "total_orders": "20", "cancelled_orders": "2", "kpt_p80_minutes": "3.4"},
        {"order_date": "2026-08-18", "store_name": "DEL KK GK1 Online", "channel": "swiggy",
...
def test_ops_computed_missing_kpt_pairs_gives_null():
    ops_metric_rows = [
        {"order_date": "2026-08-17", "store_name": "DEL KK GK1 Online", "channel": "zomato",
         "total_orders": "0", "cancelled_orders": "0", "kpt_p80_minutes": ""},
        {"order_date": "2026-08-16", "store_name": "DEL KK GK1 Online", "channel": "swiggy",
